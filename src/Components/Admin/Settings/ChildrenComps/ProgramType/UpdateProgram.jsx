...
const UpdateProgram = ({
  HandleCloseUpdateProgram,
  FetchProgramTypeByPagination,
  EditUpdateProgramType,
}) => {
  const [programTypeData, setprogramTypeData] = useState(
    EditUpdateProgramType?.name
  );

  const HandleOnChangeProgramType = (e) => {
    e.preventDefault();
    const { value, name } = e.target;
    setprogramTypeData(value);
  };

  const HandleUpdateProgramType = async (e) => {
    e.preventDefault();
    FetchProgramTypeByPagination();
    ...
    try {
      const Response = await axios.patch(
        `${import.meta.env.VITE_BACKEND_URL}/UpdateProgramType`,
        {
          id: EditUpdateProgramType?._id,
          name: programTypeData,
        },
        {
          ...
          <div className="flex justify-center items-center mt-10">
            <div className="bg-white dark:bg-slate-900 dark:text-white rounded-xl w-[70vh] ">
              <div className=" flex justify-between items-center flex-row p-4 border-b-2 border-b-slate-400">
                <div className="text-lg font-semibold">Update Program</div>
                <button onClick={HandleCloseUpdateProgram}>
                  <IoClose
                    size={28}
              ...
              <div>
                <form onSubmit={HandleUpdateProgramType}>
                  <div className="m-4">
                    <label
                      for="Template"
                    ...
                    <input
                      type="text"
                      onChange={HandleOnChangeProgramType}
                      name="programTypeData"
                      id="programTypeData"
                      value={programTypeData}
