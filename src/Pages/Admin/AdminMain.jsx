...
const AdminMain = () => {
  ...
  const { GetDepartment } = UseGetDepartments();
  ...
  const SelectHandleUniversityData = (data) => {
    const UniversityData = {
      ...
      specialization: data?.specialization?.length || 0,
    ...
    setSelectedUniversity({
      ...
      Departments: data?.department?.length || 0,
      specializations: data?.specialization?.length || 0,
      program: data?.program?.length || 0,
    });
    ...
    setTimeout(() => {
      if (GetDataFromRedux?.id !== "") {
        GetDepartment(1, 6, "", GetDataFromRedux?.id);
      }
    }, 700);
  ...
  /* Footer height constant — update if you change the footer size below */
  const FOOTER_HEIGHT_PX = 64; // matches h-16 used for footer
  ...
  return (
    <Fragment>
      ...
      >
        {/* grid: left sidebar (300px) + main content */}
        <div
          className="grid grid-cols-[300px_1fr] w-screen"
          style={{ height: "100vh" }}
        >
          {/* LEFT: Enhanced sidebar with logo, university info, actions and nav */}
          <aside className="px-4">
            <div className="sticky top-4 flex flex-col gap-4">
              ...
              {/* Main navigation */}
              <div
                className="bg-[var(--color-surface)] rounded-2xl px-1 shadow"
                style={{ boxShadow: "var(--soft-shadow)" }}
              >
                <Sidebar />
              </div>
          ...
          <main
            ...
            /* add bottom padding equal to footer height so content is never hidden behind footer */
            style={{ paddingBottom: `${FOOTER_HEIGHT_PX + 12}px` }}
          >
            {/* Notice bar (if any) */}
            {GetUserFromRedux?.role === "Admin" &&
              NoticeListData?.filter(
                (filter) => !filter?.isStopPublish && filter?.isPublishToAll
              ).length > 0 && (
                <div className="mb-6">
                  <div
                  ...
                  >
                    ...
                    <div className="flex-1 mx-4 text-sm truncate">
                      {NoticeListData?.filter(
                        (filter) =>
                          !filter?.isStopPublish && filter?.isPublishToAll
                      )
                        ?.map((data) => data?.NoticeName)
                        .join(" • ")}
                    </div>
                    <div className="text-xs text-muted-foreground">
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b">
              <div>
                ...
                <p className="text-xs text-gray-500 mt-1">
                  Select a university to switch context for admin actions.
                </p>
              </div>
                  ...
                  {UniversityLoading ? (
                    ...
                    universities
                      ...
                      .map((data, index) => (
                        <article
                          key={index}
                          className="flex flex-col rounded-xl bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 overflow-hidden shadow-sm hover:shadow-md transition transform hover:-translate-y-1"
                        >
                          <button
