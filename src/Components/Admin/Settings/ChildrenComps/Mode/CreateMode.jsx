...
const CreateMode = ({ HandleCloseCreateMode, FetchModeByPagination }) => {
  ...
  const handleSubmitForm = async (e) => {
    ...
    try {
      const Response = await axios.post(
        `${import.meta.env.VITE_BACKEND_URL}/Create-Mode`,
        {
        ...
        <div className="flex justify-center items-center mt-10">
          <div className="bg-white dark:bg-slate-900 dark:text-white rounded-xl w-[70vh] ">
            <div className=" flex justify-between items-center flex-row px-4 py-3 border-b-2 border-b-slate-400">
              <div className="text-lg font-semibold">Create Mode</div>
              <button onClick={HandleCloseCreateMode}>
                <IoClose
                  size={28}
