...
const SubCounsellorDashboard = () => {
  ...
  return (
    <>
      <section className="top-0 dark:bg-slate-800  bg-slate-50 py-2  m-2 rounded-lg ">
        <div className="ScrollBarStyle h-[calc(100vh-125px)]  overflow-hidden overflow-y-scroll">
          <div className="px-2.5 flex  flex-col gap-6 ">
            <div className="text-base font-semibold  text-left ml-3 dark:text-white">
              Hi <span className=" font-bold"> {ReduxUserlogin?.name}</span> ,
              Welcome Back <br />
              <span className="font-bold">
                ({ReduxUserlogin?.role === "subCounsellor" && "Sub Counsellor"}{" "}
                Dashboard)
              </span>
            </div>
            ...
            <div className="flex  flex-row text-sm flex-wrap ml-2  justify-start gap-6 dark:text-black mb-6">
              {AdmissionsessionListData?.filter(
                ...
                return (
                  <div
                    key={data?.id}
                    className="bg-white rounded-xl p-6 shadow-xl border w-full max-w-xs "
                  >
            ...
            <div className=" flex  flex-col gap-6 justify-start mr-2 items-center">
              <div className="rounded-xl w-full h-auto bg-white shadow-xl border">
                ...
                <div className="text-left ml-6 mb-2 flex justify-start items-start flex-row gap-5 px-4 py-2">
                  ...
                  {LoggedUserData.allotedUniversities?.map((data) => (
                    <div className="flex justify-start items-start flex-col gap-5 text-xs">
            ...
            <div className=" mr-2 rounded-xl w-full h-auto bg-white shadow-xl border mb-4">
                ...
                <div className="text-left m-5">
                  <table className="bg-white border w-full border-gray-200 rounded">
                    ...
                    <tbody className="text-sm">
                      {CounsellorManager?.length > 0 &&
                        CounsellorManager?.map((data) => (
                      ...
                      <br />
                      {CounsellorManager?.length > 0 ? (
                        CounsellorManager.map((data, dataIndex) =>
                          data?.allotedCenter?.map((center, centerIndex) => (
                            <tr
                              key={`${data?._id}-${center?._id || centerIndex}`}
                              className="border-t"
                            >
                              <td className="px-4 py-2 capitalize">
                                {center?.role || "N/A"}
            ...
            <div className=" mr-2 rounded-xl w-full h-auto bg-white shadow-xl border mb-10">
              <div className="w-full h-auto ">
                <div className="text-left m-5 tracking-wide font-bold text-sm">
                  Help & Support
                </div>
...
export default SubCounsellorDashboard;
