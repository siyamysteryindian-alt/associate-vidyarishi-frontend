...
const ListOfExamsession = ({
  ExamsessionListData,
  ExamsessionCurrentPage,
  ExamsessionTotalPages,
  ExamsessionLimit,
  ExamsessionLoading,
  ExamsessionTotalDocs,
  FetchExamsessionByPagination,
  handleLimitChange,
  handlePageChange,
}) => {
  const [UpdateExamsessionButton, setUpdateExamsessionButton] = useState(false);

  const [EditUpdateExamsession, setEditUpdateExamsession] = useState(null);

  const HandleOpenUpdateExamsession = () => {
    setUpdateExamsessionButton(true);
  };
  const HandleCloseUpdateExamsession = () => {
    setUpdateExamsessionButton(false);
  };

  // Login permission
  const HandleUpdateStatusToggleExamsession = async (ExamsessionId, BooleanValue) => {
    try {
      const response = await axios.post(
        `${import.meta.env.VITE_BACKEND_URL}/ExamsessionisAvailableToggle`,
        { ExamsessionId, BooleanValue },
        {
  ...
  const HandleLoginStatusToggleYes = (data) => {
    HandleUpdateStatusToggleExamsession(data?._id, true);
  };
  const HandleLoginStatusToggleNo = (data) => {
    HandleUpdateStatusToggleExamsession(data?._id, false);
  };

  const HandleDeleteExamsession = (data) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        Swal.fire("Deleted Successfulyy!", "", "success");
        // Yahan par aap apne save logic ko implement kar sakte hain
  ...
  return (
    <>
      <div className="py-2.5 px-4">
        <div className="h-auto w-auto">
         ...
         w-[135vh] relative top-0 left-0 "
              ...
              ) : ExamsessionListData?.length !== 0 ? (
                ExamsessionListData?.map((data, i) => (
                  <tr
                    key={i}
                  ...
                  >
                    ...
                    <td className="px-4 py-2 flex gap-2">
                      <button
                        onClick={() => {
                          HandleOpenUpdateExamsession();
                          setEditUpdateExamsession(data);
                        }}
                      ...
                      <button
                        onClick={() => HandleDeleteExamsession(data)}
          ...
          <nav aria-label="Table navigation" className="relative">
            ...
            >
              ...
              <ul className="inline-flex items-stretch -space-x-px">
                <li>
                  <button
                    onClick={() => handlePageChange(ExamsessionCurrentPage - 1)}
                ...
                <li>
                  <div className="flex items-center justify-center py-2 px-3 leading-tight text-gray-500 bg-white border border-gray-300 hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white">
                    {ExamsessionCurrentPage + "-" + ExamsessionTotalPages}
                  </div>
                </li>
      ...
      {UpdateExamsessionButton && (
        <span>
          <UpdateExamsession
            EditUpdateExamsession={EditUpdateExamsession}
            FetchExamsessionByPagination={FetchExamsessionByPagination}
            HandleCloseUpdateExamsession={HandleCloseUpdateExamsession}
          />
        </span>
      )}
    </>
...
export default ListOfExamsession;
