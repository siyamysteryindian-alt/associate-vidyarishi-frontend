...
const CenterLayout = () => {
  ...
  const SelectHandleUniversityData = (data) => {
    ...
    // optionally you may fetch dependent data here
  ...
  // Notice fetching (re-implemented similar pattern)
  ...
  const FetchNotice = async () => {
    try {
      ...
      setNoticeListData([]);
      // keep the error quiet or toast it
    } finally {
  ...
  useEffect(() => {
    // only fetch notices once logged user and redux user id available
  ...
  const filteredNotices = NoticeListData?.filter(
    (filter) =>
      ...
      (filter?.isPublishToAll || filter?.isPublishToCenter)
  );

  const shouldShowBanner =
    Location.pathname === `/center/dashboard` &&
    GetUserFromRedux?.role === "center" &&
    ...
    <Fragment>
      ...
      >
        ...
        >
          ...
          >
            {/* Notice bar (if any) */}
            {GetUserFromRedux?.role === "center" && shouldShowBanner && (
              <div className="mb-6">
                <div
            ...
            <section className="bg-none">
              {Location.pathname === `/center` ? (
                <div className="w-full h-[60vh] flex flex-col items-center justify-center gap-6">
                  <h2 className="text-3xl font-semibold">Welcome Center</h2>
                  <button
                    onClick={() => NavigateTo("/center/dashboard")}
                ...
                <div>
                  {/* You can show skeleton while logged user loader */}
                  {GetLoggedUserLoader ? (
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b">
              <div>
                ...
                <p className="text-xs text-gray-500 mt-1">
                  Select a university to switch context for center actions.
                </p>
              </div>
