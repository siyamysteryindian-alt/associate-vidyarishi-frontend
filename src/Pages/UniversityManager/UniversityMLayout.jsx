...
const UniversityMLayout = () => {
  ...
  const { GetCenter } = UseGetCenterSubCenter();
  useEffect(() => {
    if (GetDataFromRedux?.id) GetCenter();
  }, [GetDataFromRedux?.id]); // eslint-disable-line
  ...
  return (
    <Fragment>
      ...
      {/* University change modal (drawer) */}
      {UniversityChange && (
...
export default UniversityMLayout;
