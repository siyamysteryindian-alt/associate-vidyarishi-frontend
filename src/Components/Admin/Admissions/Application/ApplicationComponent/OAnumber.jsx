...
const OAnumber = ({
  ...
  const [OAData, setOAData] = useState(CompleteStudentData?.OANumber);

  const HandleOAnumberUpdate = (e) => {
    setOAData(e.target.value);
  };

  console.log(OAData);

  const HandleSubmitOAData = async () => {
    try {
      const response = await axios.patch(
        `${import.meta.env.VITE_BACKEND_URL}/OnlineApplicationUpdate`,
        {
          StudentId: CompleteStudentData?._id,
          OnlineApplnNo: OAData,
        },
        {
  ...
  const HandleSubmitOADataDelete = async () => {
    try {
      const response = await axios.patch(
        `${import.meta.env.VITE_BACKEND_URL}/OnlineApplicationUpdate`,
        {
    ...
    <div className="fixed inset-0 z-40 bg-black/60 backdrop-blur-sm flex items-center justify-center">
      <div className="bg-white dark:bg-slate-900 dark:text-white rounded-xl w-[80vh] max-w-xl shadow-2xl border border-slate-200/70 dark:border-slate-700">
        ...
        <div className="flex items-center justify-between px-5 py-3 border-b border-slate-200 dark:border-slate-700">
          ...
          <button onClick={HandleOANumberClose}>
            <IoClose
              size={26}
        ...
        <div className="px-6 pt-5 pb-2">
          ...
          <input
            ...
            id="OAData"
            value={OAData || ""}
            onChange={HandleOAnumberUpdate}
            className="w-full px-3 py-2 rounded-md border border-gray-300 dark:border-gray-600 bg-gray-50 dark:bg-slate-800 text-sm text-gray-900 dark:text-gray-100 placeholder-gray-400 dark:placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all"
            placeholder="Enter Online Application Number"
            required
          />
          <p className="mt-1 text-[11px] text-slate-500">
            This number will be used for tracking the student's online
            application.
          </p>
        </div>
        ...
        <div className="px-6 py-4 flex items-center justify-between gap-4 border-t border-slate-200 dark:border-slate-700">
          {CompleteStudentData?.OANumber !== "" && (
            <button
              onClick={HandleSubmitOADataDelete}
