...
const SubcounsellorLayout = () => {
  ...
  // keep existing logic: persisted modal state, search, select handler etc.
  const [UniversityChange, setUniversityChange] = useState(() => {
    try {
      const saved = localStorage.getItem("UniversityChange");
  ...
  // Notice fetching kept as-is in logic by user (only UI adapted)
  ...
  const FetchNotice = async () => {
    try {
      ...
      if (response?.data?.success) setNoticeListData(response?.data?.data);
      else setNoticeListData([]);
    } catch (err) {
      setNoticeListData([]);
    } finally {
  ...
  useEffect(() => {
    if (GetUserFromRedux?.id) FetchNotice();
  }, [GetUserFromRedux?.id]); // eslint-disable-line

  const filteredNotices = NoticeListData?.filter(
    (f) => !f?.isStopPublish && (f?.isPublishToAll || f?.isPublishToCounsellor)
  );
  const shouldShowBanner =
    (Location.pathname === `/sub-counsellor/dashboard` ||
      Location.pathname === `/sub-counsellor`) &&
    (GetUserFromRedux?.role === "subCounsellor" ||
      GetUserFromRedux?.role === "SubCounsellor") &&
  ...
  return (
    <Fragment>
              ...
              {Location.pathname === `/sub-counsellor` ? (
                <div className="w-full h-[60vh] flex flex-col items-center justify-center gap-6">
                  <h2 className="text-3xl font-semibold">
                    Welcome Sub Counsellor
                  </h2>
                  <button
                    onClick={() => NavigateTo("/sub-counsellor/dashboard")}
      ...
      {/* University change modal (drawer) - UI only, logic preserved */}
      {UniversityChange && (
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b">
              <div>
                ...
                <p className="text-xs text-gray-500 mt-1">
                  Select a university to switch context for sub-counsellor
                  actions.
                </p>
              </div>
...
export default SubcounsellorLayout;
