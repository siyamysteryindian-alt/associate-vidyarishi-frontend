...
const AccountantLayout = () => {
  ...
  useEffect(() => {
    if (GetFromReduxUser?.id) FetchNotice();
  }, [GetFromReduxUser?.id]); // eslint-disable-line

  const filteredNotices = NoticeListData?.filter(
    (f) => !f?.isStopPublish && (f?.isPublishToAll || f?.isPublishToCenter)
  );
  const shouldShowBanner =
    Location.pathname === `/accountant/dashboard` &&
    GetFromReduxUser?.role === "Accountant" &&
  ...
  const hasValidNotifications = NotifyData?.some(
    (d) =>
      ...
      !d?.isStopPublish &&
      (d?.isPublishToCenter ||
        d?.isPublishToSubCenter ||
        d?.isPublishToCounsellor ||
        d?.isPublishToSubCounsellor ||
        d?.isPublishToAll)
  );
            ...
            <section className="bg-none">
              {Location.pathname === `/accountant` ? (
                <div className="w-full h-[60vh] flex flex-col items-center justify-center gap-6">
                  <h2 className="text-3xl font-semibold">Welcome Accountant</h2>
                  <button
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b">
              <div>
                ...
                <p className="text-xs text-gray-500 mt-1">
                  Select a university to switch context for accountant actions.
                </p>
              </div>
